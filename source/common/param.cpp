...
void x265_print_params(x265_param* param)
{
    ...
    if (param->scenecutThreshold && param->keyframeMax != INT_MAX) 
        x265_log(param, X265_LOG_INFO, "Keyint min / max / scenecut / bias      : %d / %d / %d / %.2lf \n",
        ...
        x265_log(param, X265_LOG_INFO, "Keyint min / max / scenecut  : %d / %d / %d\n",
                 param->keyframeMin, param->keyframeMax, param->bHistBasedSceneCut);
    else if (param->keyframeMax == INT_MAX)
        x265_log(param, X265_LOG_INFO, "Keyint min / max / scenecut             : disabled\n");
...
#define BOOL(param, cliopt) \
    ...
    if (p->bEnableHME)
    {
        ...
        s += sprintf(s, " hmerange-level-L0,L1,L2=%d,%d,%d", p->hmeRange[0], p->hmeRange[1], p->hmeRange[2]);
    }
    ...
    if (p->bEnableSceneCutAwareQp)
        s += sprintf(s, " fwd-scenecut-window=%d fwd-ref-qp-delta=%.2f fwd-nonref-qp-delta=%.2f bwd-scenecut-window=%d bwd-ref-qp-delta=%.2f bwd-nonref-qp-delta=%.2f", p->fwdMaxScenecutWindow, p->fwdRefQpDelta[0], p->fwdNonRefQpDelta[0], p->bwdMaxScenecutWindow, p->bwdRefQpDelta[0], p->bwdNonRefQpDelta[0]);
